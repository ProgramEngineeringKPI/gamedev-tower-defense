...
namespace strange.extensions.implicitBind.impl
{
	public class ImplicitBinder : IImplicitBinder
	{
			...
			if (Assembly != null)
			{
				...
				foreach (Type type in typesInNamespaces)
				{
					...
					if (implementedBy.Any())
					{
						...
						{
							implementedByBindings.Add(new ImplicitBindingVO(type, implBy.DefaultType, implBy.Scope == InjectionBindingScope.CROSS_CONTEXT, null, implBy.IsSingletone));
						}
						else
						{
					...
					if (implements.Any())
					{
						...
						foreach (Implements impl in implements)
						{
                            ...
                            isSingleton = isSingleton || impl.IsSingleton;
							name = name ?? impl.Name;
						}

					    ImplicitBindingVO thisBindingVo = new ImplicitBindingVO(bindTypes, type, isCrossContext, name, isSingleton);
		...
		private sealed class ImplicitBindingVO
		{
			...
			public ImplicitBindingVO(Type bindType, Type toType, bool isCrossContext, object name, bool isSingleton)
			{
			...
			public ImplicitBindingVO(List<Type> bindTypes, Type toType, bool isCrossContext, object name, bool isSingleton)
			{
				BindTypes = bindTypes;
				ToType = toType;
				IsCrossContext = isCrossContext;
